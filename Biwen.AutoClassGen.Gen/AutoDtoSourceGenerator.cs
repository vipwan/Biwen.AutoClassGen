...
public class AutoDtoSourceGenerator : IIncrementalGenerator
{
    ...
    private static List<AutoDtoMetadata> GetMeta(ImmutableArray<GeneratorAttributeSyntaxContext> nodes, Compilation compilation, bool isGeneric = false)
    {
        ...
        foreach (var syntaxContext in nodes.AsEnumerable())
        {
    ...
    private record EntityInfo(string EntityName, ITypeSymbol? EntityTypeSymbol);

    /// <summary>
    /// 从属性中提取实体名称和类型信息
    /// </summary>
    private static EntityInfo? ExtractEntityInfo(AttributeSyntax attributeSyntax, SemanticModel semanticModel, bool isGeneric)
    {
        if (isGeneric)
        {
                                     ...
                                     Dictionary<string, AutoDtoMetadata> nestedDtoTypesToGenerate)
    {
        ...
        // 对于跨库场景，不自动生成嵌套DTO，避免生成不必要的类型
        // 只有在isComplex为true且当前类型在同一编译单元中时才生成嵌套DTO
        ...
        // 如果不是复杂DTO或者类型不在当前编译单元中，返回false
        return false;
    }
        ...
        foreach (var prop in type.GetMembers().OfType<IPropertySymbol>())
        {
            ...
            if (typeToCheck != null && IsComplexType(typeToCheck))
            {
                ...
                if (TryGetDtoType(compilation, typeToCheck, currentNamespace, isComplex, out INamedTypeSymbol? dtoSymbol, out string dtoTypeName, nestedDtoTypesToGenerate))
                {
                    ...
                    {
                        // 使用保存的类型符号进行递归处理
                        var nestedMetadata = nestedDtoTypesToGenerate[typeToCheck.Name];
                        if (nestedMetadata.EntityTypeSymbol != null)
                        {
                            ProcessTypeProperties(compilation, nestedMetadata.EntityTypeSymbol, currentNamespace,
...
#pragma warning disable CA1031 // 不捕获常规异常类型
            ...
            {
                // 使用保存的类型符号
                var symbol = metadata.EntityTypeSymbol;
            ...
            try
            {
                ...
                classTemp = classTemp.Replace("$isRecord", metadata.IsRecord ? "record class" : "class");
                {
                    ...
                    void GenProperty(ITypeSymbol @type, bool isBaseType = false, int currentNestingLevel = 0, bool shouldGenerateProperty = true)
                    {
                        ...
                        @type.GetMembers().OfType<IPropertySymbol>().ToList().ForEach(prop =>
                        {
                            ...
                            if (prop.ContainingType.Name != @type.Name)
                            {
                                return; // 跳过不属于当前类型的属性
                            }
                            ...
                            if (!metadata.Escapes.Contains(prop.Name))
                            {
                                ...
                                if (!shouldGenerateProperty)
                                {
                                    ...
                                    if (currentNestingLevel < maxLevel2)
                                    {
                                        ...
                                        if (typeToCheck != null && IsComplexType(typeToCheck))
                                        {
                                            ...
                                            {
                                                // 处理嵌套类型
                                                if (currentNestingLevel + 1 < maxLevel2)
                                                {
                                                    // 递归处理嵌套类型，但不生成属性
                                                    GenProperty(typeToCheck, false, currentNestingLevel + 1, false);
                                                }
                                            }
                                        }
                                    }
                                ...
                                // body:
                                bodyInnerBuilder.AppendLine($"/// <inheritdoc cref=\"{@type.ToDisplayString()}.{prop.Name}\" />");
                                // 添加特性
